...
EXTERN_C DLLEXPORT int sim(WolframLibraryData libData, mint Argc, MArgument *Args, MArgument Res) {
    // Extract input arguments
    const int n_wild = MArgument_getReal(Args[0]);
    const int n_drive = MArgument_getReal(Args[1]);
    const int max_pop_size = MArgument_getReal(Args[2]);
    const int n_gen = MArgument_getReal(Args[3]);
    const double avg_offspring = MArgument_getReal(Args[4]);
    const double drive_rate = MArgument_getReal(Args[5]);
    const double recomb_rate = MArgument_getReal(Args[6]);
    const double selection_drive = MArgument_getReal(Args[7]);
    const double selection_payload = MArgument_getReal(Args[8]);
    const double selection_toxin = MArgument_getReal(Args[9]);
    const double migration = MArgument_getReal(Args[10]);
    const int n_patches = MArgument_getReal(Args[11]);

    // Set random seed
    rnd::set_seed();

    // Initialize multiplicative payload
    const std::vector<double> type_payload = initialize_multiplicative_payload(selection_drive, selection_payload, selection_toxin);

    // Initialize parent populations for each patch
    std::vector<std::vector<int>> parents(n_patches);
    parents[0] = initialize_pop(n_wild, n_drive);
    for (int patch = 1; patch < n_patches; ++patch) {
        parents[patch] = initialize_pop(max_pop_size, 0);
    }

    // Store simulation results
    std::vector<std::vector<std::vector<int>>> simulation_results(n_patches);
    for (int patch = 0; patch < n_patches; ++patch) {
        simulation_results[patch].push_back(parents[patch]);
    }

    // Simulate population dynamics for each generation
    std::vector<std::vector<int>> n_migrate(n_patches);
    for (int gen = 0; gen < n_gen - 1; ++gen) {
        for (int patch = 0; patch < n_patches; ++patch) {
            n_migrate[patch].clear();
        }

        for (int patch = 0; patch < n_patches; ++patch) {
            // Offspring generation after selection
            std::vector<int> offspring_after_selection(n_genotypes, 0);
            int total = 0;
            for (int i = 0; i < n_genotypes; ++i) {
                offspring_after_selection[i] = rnd::poisson(parents[patch][i] * avg_offspring * type_payload[i]);
            ...
            // Offspring generation after mating
            ...
            parents[patch] = offspring_after_mating;

            // Store population after each generation
            simulation_results[patch].push_back(parents[patch]);

            // Select individuals for migration
            int n_migrating_individuals = rnd::binomial(total, migration);
            for (int i = 0; i < n_migrating_individuals; ++i) {
                const int migrate_individual = dist.sample();
                dist[migrate_individual] = dist[migrate_individual] - 1;
                --total;
                n_migrate[patch].push_back(migrate_individual);
            }
        }

        // Move individuals to neighboring patches
        for (int patch = 0; patch < n_patches; ++patch) {
            for (size_t i = 0; i < n_migrate[patch].size(); ++i) {
                int individual = n_migrate[patch][i];
                if (rnd::uniform() < 0.5) { // 50/50 chance to move left
                    if (patch > 0) {
                        parents[patch - 1][individual]++;
                    } else {
                        // If no patch to the left, move to the last patch (patch 2)
                        parents[n_patches - 1][individual]++;
                    }
                } else { // 50/50 chance to move right
                    if (patch < n_patches - 1) {
                        parents[patch + 1][individual]++;
                    } else {
                        // If no patch to the right, move to the first patch (patch 0)
    ...
    // Create and initialize the result tensor
    
    MTensor m_tensor;
    mint dims[3] = {n_patches, static_cast<mint>(simulation_results[0].size()), static_cast<mint>(simulation_results[0][0].size())};
    libData->MTensor_new(MType_Integer, 3, dims, &m_tensor);
    mint *data = libData->MTensor_getIntegerData(m_tensor);

    // Copy simulation results into the tensor
    for (size_t patch = 0; patch < n_patches; ++patch) {
        for (size_t gen = 0; gen < simulation_results[patch].size(); ++gen) {
            for (size_t j = 0; j < simulation_results[patch][gen].size(); ++j) {
                data[patch * simulation_results[0].size() * simulation_results[0][0].size() + gen * simulation_results[0][0].size() + j] = simulation_results[patch][gen][j];
            }
        }
    }

    // Set the result to be returned to Mathematica
    MArgument_setMTensor(Res, m_tensor);
