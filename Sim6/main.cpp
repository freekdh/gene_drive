...
double TestForInvasion(const std::vector<double> &);
bool is_fixed(std::vector<double> &parents, const std::vector<double> &type_payload, const double &drive_rate, const double &recomb_rate);
...
int main(int argc, char *argv[]){
    ...
    std::vector<double> critical_migration(n_gen,0.0);
    for(int i =0; i < n_gen; ++i)
    {
        critical_migration[0] = TestForInvasion(parents);
...
double TestForInvasion(const std::vector<double> &pop){
