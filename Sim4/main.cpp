...
void print(std::ofstream &output_file, const double &prob,  const double &, const double &, const double &, const int &n_drive, const double &spl);
...
int main(int argc, char *argv[]){
    ...
    output_file.open ("./Data/sim_results.csv", std::ofstream::app);
    ...
    int extinction = 0, no_invasion = 0, invasion = 0, n_gen_to_extinction = 0;
    for(int rep = 0; rep < n_rep; ++rep){
        ...
        while(true)
            ...
            if(is_extinct(parents)){
                ++extinction;
                n_gen_to_extinction+= gen;
                // conditioning on extinction how long does it take?
                break;
            }
            if(parents[0]==max_pop_size){
                ++no_invasion;
                break;
            }
            if(parents[51]==max_pop_size){
                ++invasion;
                break;
            }
            if(gen>=n_gen_max){
                std::cout << "warning, reached n_gen_max" << std::endl;
                std::cout << "wildtype: " << parents[0] << std::endl;
    ...
    // collect data 
    double p_extinct = (double)extinction/(double)n_rep;
    double p_invasion = (double)invasion/(double)n_rep;
    double p_no_invasion = (double)no_invasion/(double)n_rep;
    double avg_gen_to_extinction = 0.0;
    if(extinction!=0){
        avg_gen_to_extinction = (double)n_gen_to_extinction/(double)extinction;
    }
    print(output_file, p_extinct,p_invasion, p_no_invasion, avg_gen_to_extinction, n_drive,selection_payload);
...
void print(std::ofstream &output_file, const double &p_extinct,  const double &p_invasion, const double &p_no_invasion, const double &avg_gen_to_extinction, const int &n_drive, const double &spl){
    output_file << n_drive << output_file.fill() << spl << output_file.fill() << p_extinct << output_file.fill() << p_invasion << output_file.fill() << p_no_invasion << output_file.fill();
    if(avg_gen_to_extinction==0.0){
        output_file << "NA" << std::endl;
    }
    else{
        output_file << avg_gen_to_extinction << std::endl;
    }
}
